...
try {
    ...
    if (!$id || !$admin_name || !$age || !$date_of_birth || !$blood_group || !$phone_number || !$address || !$email || !$username) {
    ...
    $check_query = "SELECT id FROM admin_registration WHERE (username = ? OR email = ?) AND id != ?";
    ...
    $query = "UPDATE admin_registration SET 
              admin_name = ?, 
              age = ?, 
              date_of_birth = ?, 
              blood_group = ?, 
    ...
    if ($stmt->affected_rows === 0) {
        throw new Exception('No changes were made to the admin information');
    }

    sendJsonResponse(true, 'Admin information updated successfully');
